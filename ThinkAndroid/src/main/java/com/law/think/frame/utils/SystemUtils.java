...
public class SystemUtils {
	...
	public static String getAppName(String packageName) {
		...
		if (packageName == null) {
			packageName = ThinkCore.getInstance().getApplication().getPackageName();
		}

		try {
			PackageManager packageManager = ThinkCore.getInstance().getApplication().getPackageManager();
			PackageInfo packageInfo = packageManager.getPackageInfo(packageName, 0);
			applicationName = ThinkCore.getInstance().getApplication().getString(packageInfo.applicationInfo.labelRes);
		} catch (Exception e) {
			Logger.e(e);
			applicationName = "";
		}
	...
	public static boolean isRelease(String signatureString) {
		...
		try {
			PackageManager pm = ThinkCore.getInstance().getApplication().getPackageManager();
			PackageInfo pi = pm.getPackageInfo(ThinkCore.getInstance().getApplication().getPackageName(),
	...
	public static HashMap<String, Integer> getDisplayMetrics() {
		if (map == null) {
			...
			Display display = ((WindowManager) ThinkCore.getInstance().getApplication()
	...
	public static int dip2px(float dipValue) {
		final float scale = ThinkCore.getInstance().getApplication().getResources().getDisplayMetrics().density;
	...
	public static int getBarHeight() {
		...
		try {
			...
			sbar = ThinkCore.getInstance().getApplication().getResources().getDimensionPixelSize(x);
		} catch (Exception e1) {
			Logger.e(e1);
		}
		return sbar;
	...
	public static void Vibrate(long milliseconds) {
		Vibrator vib = (Vibrator) ThinkCore.getInstance().getApplication().getSystemService(Service.VIBRATOR_SERVICE);
	...
	public static void init() {
		mTm = (TelephonyManager) ThinkCore.getInstance().getApplication().getSystemService(Context.TELEPHONY_SERVICE);
		...
		try {
			ConnectivityManager cm = (ConnectivityManager) ThinkCore.getInstance().getApplication()
	...
	private static String getDeviceId() {
		return Secure.getString(ThinkCore.getInstance().getApplication().getContentResolver(), Secure.ANDROID_ID);
	}
	...
	public static String getPhoneSettings() {
		...
		String str = Secure.getString(ThinkCore.getInstance().getApplication().getContentResolver(),
				Secure.BLUETOOTH_ON);
		buf.append("蓝牙:");
		if (str.equals("0")) {
			buf.append("禁用");
		...
		//
		str = Secure.getString(ThinkCore.getInstance().getApplication().getContentResolver(), Secure.BLUETOOTH_ON);
		buf.append("WIFI:");
		buf.append(str);

		str = Secure.getString(ThinkCore.getInstance().getApplication().getContentResolver(),
	...
	public static void clearUserData(final Context context) {
		try {
			...
			Object iPackageDataObserverObject = Proxy.newProxyInstance(ThinkCore.class.getClassLoader(),
					new Class[] { iPackageDataObserverClass }, new InvocationHandler() {
	...
	public static void exit(Activity context) {
		...
		if (currentVersion > android.os.Build.VERSION_CODES.ECLAIR_MR1) {
			...
			ActivityManager am = (ActivityManager) ThinkCore.getInstance().getApplication()
					.getSystemService(Activity.ACTIVITY_SERVICE);
			am.restartPackage(ThinkCore.getInstance().getApplication().getPackageName());
		}
