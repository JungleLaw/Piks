package com.law.think.frame.prefs;
 ...
 * <p/>
 * 保存PrefArrayList的key是父Object的key+$$$$+fieldKey+_arraylist_+index，如
 * "Sample$$$$myArrayList_arraylist_3";
 * PrefArrayList的长度需要保存在一个Key里，key名为父Object的key
 * +$$$$+fieldKey+_arraylist_length，如
 * "Sample$$$$myArrayList_arraylist_length";四个$
 */
...
public class AnyPref {
	...
	public static SharedPrefs getDefault() {
		return new SharedPrefs(mContext.getPackageName() + "_preferences",
				mContext);
	}
	...
	public static void clear(Class clazz, String prefName) {
		...
		for (Field field : PrefUtil.getFields(clazz)) {
			if (PrefUtil.isSubPref(field)) {
				clear(field.getType(),
						prefName + "$$$" + PrefUtil.getKeyForField(field));
	...
	public static <T> T get(Class<T> clazz, String prefName, boolean nullable) {
		...
		SharedPreferences preferences = mContext.getSharedPreferences(prefName,
	 ...
	 *            变量父对象
	 * @param prefName
	 *            父对象保存SharedPreferences的name
	 */
	private static void put(SharedPreferences.Editor editor, Field field,
			Object object, String prefName) {
	 ...
	 *            父对象保存SharedPreferences的name
	 */
	private static void get(SharedPreferences preferences, Field field,
			Object object, String prefName) {
			...
			case "int":
				field.setInt(
						...
						preferences.getInt(key,
								(int) PrefUtil.getDefaultNumber(cacheKey)));
				break;
			case "float":
				field.setFloat(
						object,
						preferences.getFloat(key,
								(float) PrefUtil.getDefaultNumber(cacheKey)));
				break;
			case "long":
				field.setLong(
						object,
						preferences.getLong(key,
								(long) PrefUtil.getDefaultNumber(cacheKey)));
				break;
			case "boolean":
				field.setBoolean(
						object,
						preferences.getBoolean(key,
								PrefUtil.getDefaultBoolean(cacheKey)));
				break;
			case "java.lang.String":
				field.set(
						object,
						preferences.getString(key,
								PrefUtil.getDefaultString(cacheKey)));
				break;
			default:
				if (PrefUtil.isSubPref(field)) {
					field.set(
							object,
							get(field.getType(), prefName + "$$$" + key,
									PrefUtil.isPrefSubNullable(field)));
				} else if (PrefUtil.isArrayListPref(field)) {
					field.set(object,
							getArrayList(preferences, field, prefName));
				} else if (PrefUtil.isFieldStringSet(field)) {
					field.set(
							object,
							preferences.getStringSet(key,
									PrefUtil.getDefaultStringSet(cacheKey)));
				}
				break;
			}
		} catch (IllegalAccessException e) {
	...
	private static ArrayList getArrayList(SharedPreferences preferences,
			Field field, String prefName) {
		int length = preferences.getInt(
				PrefUtil.getArrayListLengthKey(field, prefName), 0);
	...
	private static void putArrayList(SharedPreferences.Editor editor,
			ArrayList arrayList, Field field, String prefName) {
		for (int i = 0; i < arrayList.size(); i++) {
			put(arrayList.get(i),
					PrefUtil.getArrayListItemKey(field, prefName, i));
		}
		editor.putInt(PrefUtil.getArrayListLengthKey(field, prefName),
				arrayList.size());
	}
