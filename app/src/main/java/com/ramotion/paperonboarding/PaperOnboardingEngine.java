...
public class PaperOnboardingEngine implements PaperOnboardingEngineDefaults {
    ...
    // params for Pager position calculations, virtually final, but initializes
    // in onGlobalLayoutListener
    ...
    public PaperOnboardingEngine(View rootLayout, ArrayList<PaperOnboardingPage> contentElements, Context appContext) {
        ...
        mRootLayout.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                mPagerElementNormalSize = Math.min(mPagerIconsContainer.getChildAt(0).getHeight(), mPagerIconsContainer.getChildAt(mPagerIconsContainer.getChildCount() - 1).getHeight());
    ...
    /**
     * Calculate new position for pager without using pager's current
     * position(like .getX()) this method allows to avoid incorrect position
     * values while animation of pager in progress
     *
    ...
    protected int calculateNewPagerPosition(int newActiveElement) {
        ...
        if (newActiveElement <= 0)
            newActiveElement = 1;
        int pagerActiveElemCenterPosX = mPagerElementActiveSize / 2 + newActiveElement * mPagerElementLeftMargin + (newActiveElement - 1) * (mPagerElementNormalSize + mPagerElementRightMargin);
    ...
    /**
     * @param prev set true to animate onto previous content page (default is
     *             false - animating to next content page)
     */
    protected void toggleContent(boolean prev) {
        ...
        AnimatorSet contentTextShowAnimation = createContentTextShowAnimation(mContentTextContainer.getChildAt(mContentTextContainer.getChildCount() - 2), newContentText);
        ...
        AnimatorSet contentIconShowAnimation = createContentIconShowAnimation(mContentIconContainer.getChildAt(mContentIconContainer.getChildCount() - 2), newContentIcon);
    ...
    protected Animator createContentCenteringVerticalAnimation(View newContentText, View newContentIcon) {
        ...
        Animator centerContentAnimation = ObjectAnimator.ofFloat(mContentCenteredContainer, "y", mContentCenteredContainer.getY(), (mContentRootLayout.getHeight() - newHeightOfContent) / 2);
     ...
     * @return configured pager icon with selected drawable and selected state
     * (active or inactive)
     */
    ...
    protected ViewGroup createContentTextView(PaperOnboardingPage PaperOnboardingPage) {
        ...
        if (PaperOnboardingPage.isEntry()) {
            contentTextView.getChildAt(2).setVisibility(View.VISIBLE);
            if (onEntryClickListener != null)
                contentTextView.getChildAt(2).setOnClickListener(onEntryClickListener);
        } else {
            contentTextView.getChildAt(2).setVisibility(View.INVISIBLE);
        }
    ...
    public void setOnEntryClickListener(View.OnClickListener onEntryClickListener) {
