package com.law.piks.widget.bottomsheet;
 ...
 * <p>
 * A list can support submenus, and will include a divider and header for them
 * where appropriate. Grids currently don't support submenus, and don't in the
 * Material Design spec either.
 * </p>
 */
...
public class MenuSheetView extends FrameLayout {
    ...
    public MenuSheetView(final Context context, final MenuType menuType, @Nullable final CharSequence title, final OnMenuItemClickListener listener) {
        ...
        absListView = (AbsListView) findViewById(menuType == MenuType.GRID ? R.id.grid : R.id.list);
        if (listener != null) {
     ...
     * <p/>
     * Adapted from the Design support library's NavigationMenuPresenter
     * implementation
     */
            ...
            if (item.isVisible()) {
                if (item.hasSubMenu()) {
                    ...
                    if (subMenu.hasVisibleItems()) {
                        ...
                        // Add one more separator to the end to close it off if
                        // we have more items
    ...
    /**
     * @return The current {@link Menu} instance backing this sheet. Note that
     * this is mutable, and you should call {@link #updateMenu()} after
     * any changes.
     */
    public Menu getMenu() {
        return this.menu;
    ...
    /**
     * Invalidates the current internal representation of the menu and rebuilds
     * it. Should be used if the developer dynamically adds items to the Menu
     * returned by {@link #getMenu()}
     */
    public void updateMenu() {
        // Invalidate menu and rebuild it, useful if the user has dynamically
        // added menu items.
        prepareMenuItems();
    }
    ...
    public void setTitle(CharSequence title) {
        if (!TextUtils.isEmpty(title)) {
            ...
            absListView.setPadding(absListView.getPaddingLeft(), originalListPaddingTop + BottomSheetUtils.dp2px(getContext(), 8f), absListView.getPaddingRight(), absListView.getPaddingBottom());
        }
    }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            switch (viewType) {
                case VIEW_TYPE_NORMAL:
                    ...
                    if (convertView == null) {
                        convertView = inflater.inflate(menuType == MenuType.GRID ? R.layout.sheet_grid_item : R.layout.sheet_list_item, parent, false);
